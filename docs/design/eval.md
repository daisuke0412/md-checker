...
1. LLM への窓口（`infra/llm.run`）が `log_to_eval=True`（既定）で Claude を呼んだ直後に、**送ったリクエストそのまま**と**返ってきた応答そのまま**をペアで記録する。固定パイプライン（`complete` 経由）もエージェント（`run` 直接）も、この同じ窓口を通るため自動で記録対象になる。
