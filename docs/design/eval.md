...
md-checker の **LLM 出力評価**（分析パイプラインが LLM に出させた判定の品質を、後から自動採点する処理）のコード設計をまとめます。分析パイプライン（[pipeline.md](pipeline.md)）が動くたびに LLM 入出力をログに残し、後で別の LLM（評価者）にそのログを採点させるまでの処理ロジックを記述します。全体の中での位置づけは [../architecture.md](../architecture.md) の「1.3 LLM 出力評価」を参照。
...
1. **評価用ログ記録** — エージェントの LLM 呼び出しのたびに、その入力（リクエスト）と出力（応答）をペアで日次ファイルに追記する。
...
```mermaid
flowchart LR
    pipe["分析パイプライン<br/>(LLM 実行)"] -->|"入出力ペア"| log[("checker LLM ログ<br/>checker/llm_io_YYYYMMDD.jsonl")]
...
採点処理は分析パイプラインと**同じ LLM 窓口（`infra/llm.run`）**を使うが、**`log_to_eval=False`** で呼ぶことで、採点呼び出しの入出力を checker の評価対象ログ（`logs/checker/`）に残さない。判定（checker）側は `log_to_eval=True`（既定）なので自動記録され、採点（judge）側だけが記録から外れる。judge 自身の LLM 入出力は、別途 `logs/judge/llm_io_YYYYMMDD.jsonl` に記録する（採点経路のトレース用）。

> もし採点呼び出しも checker の評価対象ログに記録すると、次の採点でその採点ログ自身が採点対象に混ざってしまう。評価対象は「分析パイプラインの判定」だけに限るため、`log_to_eval` フラグで採点経路を本番経路から切り離す。この分離は崩さないこと。

## 2. 評価用ログ記録

エージェントの LLM 呼び出し（[agent.md](agent.md) 4.2）のたびに、その入出力をペアで 1 行追記する。

1. LLM への窓口（`infra/llm.run`）が `log_to_eval=True`（既定）で Claude を呼んだ直後に、**送ったリクエストそのまま**と**返ってきた応答そのまま**をペアで記録する。エージェント（`run` 直接）はこの同じ窓口を通るため自動で記録対象になる。
   - 記録は LLM への窓口からのみ行う（LLM 機能に閉じた関心事として `infra/llm.py` 内に置く）。
2. 1 ペア＝1 レコードを、**日次ファイル**（`logs/checker/llm_io_YYYYMMDD.jsonl`）に追記する（[4.1](#41-保存形式)）。
3. ログ書き込みの失敗で**本処理（LLM 呼び出し）を止めない**。書き込みに失敗しても警告を出すだけで握りつぶし、エージェント側の結果は通常どおり返す。
...
1. 採点対象の**日付**を指定して、その日の checker LLM ログ（`logs/checker/llm_io_YYYYMMDD.jsonl`）を読む。指定が無ければ当日を対象にする。
...
> プロンプト定型文を外部ファイルにするのは、エージェントのプロンプト（[agent.md](agent.md) 9）と同じ方針。評価観点の調整がコードを触らずにできる。
...
採点結果も自由文ではなく、**専用 tool スキーマに沿った構造化データ**で受け取る。評価者に当該ツールの呼び出しを強制し、その入力をそのまま採点結果として扱う（[agent.md](agent.md) 3.3 と同じ方式）。
...
1. 完成した採点プロンプトと採点 tool スキーマ（`JUDGE_TOOL`）を LLM（評価者）に渡し、ツール呼び出しの入力（構造化採点）を受け取る。
2. 採点呼び出しは `infra/llm.run` を **`log_to_eval=False`** で呼び、checker の評価対象ログ（`logs/checker/`）には残さない（[1.4](#14-評価用ログと本番経路の分離)）。judge 自身の入出力トレースは `logs/judge/llm_io_YYYYMMDD.jsonl` に別途記録する。
...
| ファイル | 内容 | 書き込み方 |
|---|---|---|
| `logs/checker/llm_io_YYYYMMDD.jsonl` | checker の LLM 入出力ペア（評価の素材） | 呼び出しのたびに 1 行**追記** |
| `logs/judge/scored_YYYYMMDD.jsonl` | 採点結果 | 採点実行ごとに**一括書き出し**（上書き） |
| `logs/judge/llm_io_YYYYMMDD.jsonl` | judge 自身の LLM 入出力ペア（採点経路のトレース） | 採点のたびに 1 行**追記** |

1. いずれも **1 行 1 レコードの JSON Lines**。日本語はエスケープせずそのまま保存する。
2. ファイル名の `YYYYMMDD` は日付。checker LLM ログは記録日、採点結果は採点対象日に対応する。
...
| パラメータ | 既定値 | 説明 |
|---|---|---|
| 採点モデル | Claude（Anthropic） | 採点（評価者）に使うモデル。分析パイプラインと同じ `CLAUDE_MODEL` を共用する |
| 採点対象日付 | 当日 | 採点する checker LLM ログの日付。指定が無ければ当日（[3.1](#31-対象ログの選択)） |
| checker LLM ログ出力先 | `logs/checker/` | 採点の素材（checker の LLM 入出力ペア）の置き場所（`CHECKER_LLM_LOG_DIR`） |
| 採点結果・judge ログ出力先 | `logs/judge/` | 採点結果 `scored_*` と judge 自身の入出力 `llm_io_*` の置き場所（`JUDGE_LOG_DIR`） |
