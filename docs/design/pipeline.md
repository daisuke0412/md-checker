...
md-patrol の **検索フェーズ**（ユーザークエリのたびに動く処理）のコード設計をまとめます。ユーザーが入力した文章に対し、関連する既存記載を集め、LLM に判定させて回答を返すまでの処理ロジックを記述します。全体の中での位置づけは [../architecture.md](../architecture.md) の「1.2 md-patrol エージェント」を参照。
...
1. 各候補に **1 始まりの通し番号**を振り、「番号・ファイル名・見出しパス・本文」を 1 ブロックにまとめる。
2. 全候補のブロックを連結して、プロンプトに埋め込む候補テキストにする。

> 番号を振るのは、LLM に**出典を番号で参照させる**ため。回答のファイル名・見出しを、渡した候補に存在するものだけに縛り、実在しない出典の捏造（ハルシネーション）を防ぐ。
...
| パラメータ | 既定値 | 説明 |
|---|---|---|
| 候補取得件数 k | 8 | RAG 検索から取り寄せ、LLM に渡す候補数（[3](#3-候補取得rag-検索)）。多いほど取りこぼしは減るが入力トークンが増える |
| 最大入力文字数 | 1200 | 受け付ける入力テキストの上限（[2](#2-入力受付)）。チャンク上限トークンに合わせている。ファイル単位入力では適用しない |
| LLM モデル | Claude（Anthropic） | 判定に使うモデル（[4.4](#44-llm-呼び出し)） |
...
{
  "file": "draft.md",
  "summary": { "chunks": 7, "hit_chunks": 3, "conflicts": 4 },
  "by_chunk": [
    {
